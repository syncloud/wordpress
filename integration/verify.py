...
def module_teardown(device_host, data_dir, platform_data_dir, app_dir, log_dir, app):
    ...
    run_ssh(device_host, 'ls -la {0}/log/ > {1}/log.ls.log'.format(app_dir, TMP_DIR), password=LOGS_SSH_PASSWORD, throw=False)  
    run_ssh(device_host, '{0}/bin/wp-cli core is-installed; echo "is installed: $?"'.format(app_dir, TMP_DIR), password=LOGS_SSH_PASSWORD, throw=False, env_vars='SNAP_COMMON={0}'.format(data_dir))
