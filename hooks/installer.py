...
class Installer:
    ...
    def configure(self):
        ...
        if not isfile(install_file):
            ...
            check_output('sudo -u {0} -i -- {1}/bin/wp-cli core install --url=localhost --title=Syncloud --admin_user=admin --admin_password=admon --admin_email=info@example.com'.format(
