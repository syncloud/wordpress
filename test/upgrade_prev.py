...
def test_seed_post(device):
    device.run_ssh(
        "snap run wordpress.wp-cli post create --post_status=publish "
        "--post_title='{0}' --post_content='{1}'".format(POST_TITLE, POST_BODY))

    posts = device.run_ssh("snap run wordpress.wp-cli post list --format=csv --fields=post_title")
